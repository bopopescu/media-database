...
def AddCommonTestRunArgs(parser):
  ...
  parser.add_argument(
      ...
      'arg:value pairs to use.')
  parser.add_argument(
      '--type', '-y', choices=['instrumentation', 'monkey', 'robo'],
      help='The type of test to run (default: instrumentation).')
  parser.add_argument(
      '--app', '-a',
      help='Local or GCS path to the application binary file.')
  parser.add_argument(
      '--app-package', '-A',
...
def AddSharedCommandArgs(parser):
  ...
  parser.add_argument(
      '--results-bucket', '-b',
      help=('The name of a GCS bucket where test results will be stored '
            '(default: "cloud-test-<project-id>").'))
  parser.add_argument(
      '--results-history-name', '-H',
      help=('The name of a Tool Results history to publish test results to '
            '(default: "<app-package name> (gcloud)").'))
  parser.add_argument(
...
def AddMatrixArgs(parser):
  ...
  parser.add_argument(
      ...
      help='The device ID(s) to test against.')
  parser.add_argument(
      '--os-version-ids', '-v',
      type=arg_parsers.ArgList(min_length=1),
      metavar='OS_VERSION_ID',
      action=arg_parsers.FloatingListValuesCatcher(),
      help='The Android OS version ID(s) to test against.')
  parser.add_argument(
      '--locales', '-l',
      type=arg_parsers.ArgList(min_length=1),
      metavar='LOCALE',
      action=arg_parsers.FloatingListValuesCatcher(),
      help='The Android locale(s) to test against.')
  parser.add_argument(
      '--orientations', '-o',
      type=arg_parsers.ArgList(min_length=1,
...
def AddInstrumentationTestArgs(parser):
  ...
  parser.add_argument(
      '--test', '-t',
      help='Local or GCS path to the test binary file.')
  parser.add_argument(
      '--test-package', '-T',
  ...
  parser.add_argument(
      ...
      metavar='TEST_TARGET',
      action=arg_parsers.FloatingListValuesCatcher(),
      help=('One or more test targets to be run. Each target must be fully '
            'qualified with the package name or class name, in one of these '
            'formats:\n'
...
def AddMonkeyTestArgs(parser):
  ...
  parser.add_argument(
      ...
      help=('Fixed delay in milliseconds inserted between simulated events '
            '(default: 0).'))
  parser.add_argument(
      '--random-seed', metavar='int', type=int,
      help=('Seed value for a monkey test pseudo-random number generator '
            '(default: 0).'))
...
def AddRoboTestArgs(parser):
  ...
  parser.add_argument(
      ...
      help='The initial activity that should be used to start the app during '
      'a Robo test.')
  parser.add_argument(
      '--bootstrap-apk',
      help='The APK used for bootstrapping a Robo test (e.g., passing the login'
      ' screen).')
  parser.add_argument(
      '--bootstrap-package',
      help='The java package id for the bootstrap during a Robo test.')
  parser.add_argument(
      '--bootstrap-runner-class',
      help='The fully-qualified Java class name of the bootstrap runner.')
...
_TEST_TYPE_ARG_RULES = {
    ...
    'robo': {
        'required': [],
        'optional': ['app_initial_activity', 'bootstrap_apk',
                     'bootstrap_package', 'bootstrap_runner_class',
                     'max_depth', 'max_steps'],
        'defaults': {
            'max_depth': 50,
