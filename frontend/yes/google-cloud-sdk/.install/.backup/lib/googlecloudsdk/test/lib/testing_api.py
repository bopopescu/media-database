...
class TestingApiHelper(object):
  ...
  def __init__(self, project, args, history_id, gcs_results_root,
    ...
    self.state_names = {
        ...
        exec_states.INCOMPATIBLE_ENVIRONMENT: 'Incompatible',
  ...
  def _BuildAndroidRoboTestSpec(self):
    ...
    if self._args.bootstrap_apk:
      bootstrap_apk = self._BuildApkFileReference(self._args.bootstrap_apk)
    else:
      bootstrap_apk = None
    test = self._messages.AndroidRoboTest(
        ...
        bootstrapApk=bootstrap_apk,
        bootstrapPackageId=self._args.bootstrap_package,
        bootstrapRunnerClass=self._args.bootstrap_runner_class)
    return self._messages.TestSpecification(
  ...
  def MonitorTestExecutionProgress(self, matrix_id, test_id,
    ...
    while True:
      ...
      # Inform user of test progress from PENDING -> RUNNING -> FINISHED
      # TODO(user): we'll need more informative messages here if the Test
      # Service ever allows state RUNNING to go back to state PENDING (i.e.
      # inform the user their test was rescheduled.)
