...
"""A shared library to validate test arguments."""
...
def ValidateArgFromFile(arg_internal_name, arg_value):
  """Do checks/mutations on args from files which need extra validation.
...
def ValidateStringList(arg_internal_name, arg_value):
  ...
  """
  if isinstance(arg_value, basestring):  # convert single str to str list
    return [arg_value]
  if isinstance(arg_value, int):  # convert single int to str list
